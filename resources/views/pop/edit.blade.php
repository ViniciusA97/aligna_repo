...
<div class="page-header">
    <h1 class="page-title">Editar POP</h1>
</div>
@endsection
...
<div class="panel-body container-fluid">
    ...
    <form action="{{ route('pop.update', $data->pop->id) }}" method="POST">
        @method('PUT')
        @csrf

        <input type="hidden" name="pop_id" value="{{ $data->pop->id }}">
            ...
            <div class="col-12 col-md-7">
                <div class="row">
                    <div class="form-group col-12">
                        ...
                        placeholder="Título" autocomplete="off" value="{{ $data->pop->version->title }}" />
                        @error('title')
                ...
                <div class="row">
                    <div class="form-group col-12">
                        ...
                        placeholder="Resumo" autocomplete="off" value="{{ $data->pop->version->resume }}" />
                        @error('resume')
                        <small class="invalid-feedback">
                ...
                <div class="row">
                    <div class="form-group col-12">
                        ...
                        @enderror
                        <textarea name="description" id="inputDescription">{{ $data->pop->version->description }}</textarea>
                    </div>
                </div><!-- end row-->

                @include('partials.pop.uploads')
            </div><!--end col-left-->
            ...
            <div class="col-12 col-md-5">
                <div class="row">
                    <div class="form-group col-12 col-md-6">
                        ...
                        <select id="selectPDCA" name="pdca" data-plugin="select2" class="form-control @error('pdca') is-invalid @enderror">
                            <option selected value="">Selecione</option>
                            @foreach ($data->pdca as $item)
                                <option value="{{$item}}" {{ (collect($data->pop->version->pdca)->contains($item)) ? 'selected':'' }}>{{$item}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-12 col-md-6">
                        ...
                        <select id="selectPerfil" name="perfil" data-plugin="select2" class="form-control @error('perfil') is-invalid @enderror">
                            <option selected value="">Selecione</option>
                            @foreach ($data->perfil as $item)
                                <option value="{{$item}}" {{ (collect($data->pop->version->perfil)->contains($item)) ? 'selected':'' }}>{{$item}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-12 col-md-6">
                        ...
                        <select id="selectStatus" name="status_preenchimento" class="form-control @error('status_preenchimento') is-invalid @enderror">
                            ...
                            @foreach ($data->status['preenchimento'] as $item)
                                <option value="{{$item}}" {{ (collect($data->pop->version->status_preenchimento)->contains($item)) ? 'selected':'' }}>{{$item}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-12 col-md-6">
                        ...
                        <select id="selectStatusExecucao" name="status_execucao" class="form-control @error('status_execucao') is-invalid @enderror">
                            ...
                            @foreach ($data->status['execucao'] as $item)
                                <option value="{{$item}}" {{ (collect($data->pop->version->status_execucao)->contains($item)) ? 'selected':'' }}>{{$item}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-12">
                        ...
                        <select id="inputFunctions" name="functions" data-placeholder="Selecione as funções" class="form-control @error('functions') is-invalid @enderror" data-plugin="select2">
                            @foreach ($data->functions as $item)
                                <option value="{{$item->id}}" {{ (collect($data->functions_ids)->contains($item->id)) ? 'selected':'' }}>{{$item->title}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-12">
                        ...
                        <select id="inputProcess" name="process" data-placeholder="Selecione os Processos" class="form-control @error('process') is-invalid @enderror" data-plugin="select2">
                            @foreach ($data->process as $item)
                                <option value="{{$item->id}}" {{ (collect($data->processes_ids)->contains($item->id)) ? 'selected':'' }}>{{$item->title}}</option>
                            @endforeach
                        </select>
                ...
                <div class="row">
                    <div class="form-group col-12 col-md-6">
                        ...
                        <input type="text" class="form-control time @error('hours') is-invalid @enderror" name="hours" id="inputHrsExecucao" value="@if($data->pop->version->hours < 10) 0 @endif {{$data->pop->version->hours }} @if(strpos($data->pop->version->hours,'.') !== true) 00 @endif">
                    ...
                    <div class="form-group col-12 col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control datepicker @error('start_date') is-invalid @enderror" name="start_date" value="{{$data->pop->version->start_at->format('d/m/Y')}}">
...
<script>
    ...
    $("input[name='frequencia']").change(function(){
        let value = $("input[name='frequencia']:checked").val();
        $(".frequencia-option").hide();
        switch(value){
            case 'diario':
                $("#diario-content").show();
                break;
            case 'semanal':
                $("#semanal-content").css('display','flex');
                break;
            case 'mensal':
    ...
    @if($data->pop_recurrence)
        @if($data->pop_recurrence->rule->FREQ == 'mensal' || $data->pop_recurrence->rule->FREQ == 'anual')
            buildCalendar();
        @endif
    @endif
</script>
